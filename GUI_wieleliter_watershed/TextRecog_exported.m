classdef TextRecog_exported < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function TextrecognitionButtonPushed(app, event)
            ...
            test_bin = ~imbinarize(test);
            d = bwdist(test_bin);
            waterShed = watershed(d);
            region = regionprops(test_bin,'BoundingBox');
            result = '';
            for i = 1:length(region)
                LBPfeatures = extractLBPFeatures(test.*(waterShed==i)); 
                for j = 1:59
                ...
                result = strcat(result,string(app.nnl.dict(position)));
            end
            app.OutputEditField.Value = result;
        end
    end
